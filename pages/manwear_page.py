...
class ManWearPage(BasePageClass, ManWearPageLocator):
    ...
    def get_amount_added(self):
        ...
        value_amount_added = self.get_element_page(
            locator=self.loc_amount_product
        ).text
    ...
    def method_add_in_favorite(self, selected_product: list):
        ...
        self.get_element_page(
            locator=self.loc_btn_add_favorite
        ).click()
        # получение и запись артикула выбранного товара
        article_product = self.get_element_page(
            locator=self.loc_article_product
        ).text
        selected_product.append(article_product)
        Log.add_end_step(
            url=f'{self.driver.current_url}',
            method='method add in favorite'
        )
    ...
    def method_quick_delete_in_favorite(self):
        ...
        self.get_element_page(locator=self.loc_added_favor).click()
        Log.add_end_step(
            url=f'{self.driver.current_url}',
            method='method quick delete in favorite'
        )
    ...
    def method_delete_in_favorite(self, selected_product: list):
        ...
        # нажатие на кнопку удалить из избранного
        self.get_element_page(
            locator=self.loc_btn_added_favorite_in_cart
        ).click()
        # изменение записей выбранных товаров после удаления
        selected_product.pop()
        Log.add_end_step(
            url=f'{self.driver.current_url}',
            method='method delete in favorite'
        )
        print('добавленный товар удален из избранного')
    ...
    def method_quick_add_in_cart(self, selected_product: list):
        ...
        article_product = self.get_element_page(
            locator=self.loc_article_in_form
        ).text
        selected_product.append(article_product)
        Log.add_end_step(
            url=f'{self.driver.current_url}',
            method='method quick add in cart'
        )
    ...
    def method_add_in_cart(self, selected_product: list):
        ...
        value_price_product = self.get_element_page(
            locator=self.loc_price_in_form
        ).text
        price_product = int(
            value_price_product.replace(' ₽', '').replace(' ', '')
        )
    ...
    def method_sum_calculation_logic(self, selected_product: list):
        ...
        value_general_price = self.get_element_page(
            locator=self.loc_general_price_in_cart
        ).text
        general_price = int(
            value_general_price.replace(' ₽', '').replace(' ', '')
        )
        # сравнение результатов сыммы каждого товара с суммой заказа
        assert general_price == sum(selected_product)
        Log.add_end_step(
            url=f'{self.driver.current_url}',
            method='method sum calculation logic'
        )
